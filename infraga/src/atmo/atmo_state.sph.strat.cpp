# ifndef _ATMO_STATE_SPH_STRAT_CPP_
# define _ATMO_STATE_SPH_STRAT_CPP_
...
//-------defined by topogrpahy (not ellipsoid)-------//
//---------------------------------------------------//
double topo::z0 = 0.0;
double topo::z_max;
    ...
    if (geoac::is_topo){
        ...
        for (int nx = 0; nx < spline.length_x; nx++){
            for (int ny = 0; ny < spline.length_y; ny++){
                z_max = max(z_max, spline.f_vals[nx][ny]);
            }
        }
        z_max += 0.01;
    } else {
...
double topo::bndlyr_dsc(double r, double lat, double lon, int n){
    ...
    if (r - globe::r0 < topo::z_bndlyr) {
        X = exp(-(r - z(lat, lon)) / bndlyr_param);
        if(n > 0){
            dX = X / bndlyr_param * dz(lat, lon, n - 1);
        } else {
            dX = -X / bndlyr_param;
        }
...
double topo::bndlyr_ddsc(double r, double lat, double lon, int n1, int n2){
    ...
    if (r - globe::r0 < topo::z_bndlyr) {
        ...
        if(n1 > 0){
            dX1 = X / bndlyr_param * dz(lat, lon, n1 - 1);
        } else{
            dX1 = - X / bndlyr_param;
        }

        if(n2 > 0){
            dX2 = X / bndlyr_param * dz(lat, lon, n2 - 1);
        } else{
            dX2 = - X / bndlyr_param;
        }
    
        if(n1 > 0 && n2 > 0){
            ddX = 1.0 / bndlyr_param * (ddz(lat, lon, n1 - 1, n2 - 1) + 1.0 / bndlyr_param * dz(lat, lon, n1 - 1) * dz(lat, lon, n2 - 1)) * X;
        } else if(n1 > 0 && n2 == 0){
            ddX = - X / pow(bndlyr_param, 2) * dz(lat, lon, n1 - 1);
        } else if(n1 == 0 && n2 > 0){
            ddX = - X / pow(bndlyr_param, 2) * dz(lat, lon, n2 - 1);
        } else { 
...
double atmo::rho(double r, double lat, double lon){
    double z_eval = interp::in_interval(r - globe::r0, rho_spline.x_vals[0], rho_spline.x_vals[rho_spline.length - 1]);
    return interp::eval_f(z_eval, rho_spline);
}


double atmo::c(double r, double lat, double lon){
    double z_eval = interp::in_interval(r - globe::r0, c_spline.x_vals[0], c_spline.x_vals[c_spline.length - 1]);
    return interp::eval_f(z_eval, c_spline);
}
double atmo::dc(double r, double lat, double lon, int n){
    double z_eval, result = 0.0;
    if(n == 0){
        z_eval = interp::in_interval(r - globe::r0, c_spline.x_vals[0], c_spline.x_vals[c_spline.length - 1]);
        result = interp::eval_df(z_eval, c_spline);
    }
    return result;
}
double atmo::ddc(double r, double lat, double lon, int n1, int n2){
    ...
    if(n1 == 0 && n2 == 0){
        z_eval = interp::in_interval(r - globe::r0, c_spline.x_vals[0], c_spline.x_vals[c_spline.length - 1]);
        result = interp::eval_ddf(z_eval, c_spline);
    }
    return result;
}

double atmo::u(double r, double lat, double lon){
    double z_eval = interp::in_interval(r - globe::r0, u_spline.x_vals[0], u_spline.x_vals[u_spline.length - 1]);
    return interp::eval_f(z_eval, u_spline) * topo::bndlyr_sc(r, lat, lon);
}
double atmo::du(double r, double lat, double lon, int n){
    double z_eval = interp::in_interval(r - globe::r0, u_spline.x_vals[0], u_spline.x_vals[u_spline.length - 1]);
    double result = interp::eval_f(z_eval, u_spline) * topo::bndlyr_dsc(r, lat, lon, n);
    
    if(n == 0){
        result += interp::eval_df(z_eval, u_spline) * topo::bndlyr_sc(r, lat, lon);
    }
    return result;
}
double atmo::ddu(double r, double lat, double lon, int n1, int n2){
    double z_eval = interp::in_interval(r - globe::r0, u_spline.x_vals[0], u_spline.x_vals[u_spline.length - 1]);
    double result = interp::eval_f(z_eval, u_spline) * topo::bndlyr_ddsc(r, lat, lon, n1, n2);
    
    if(n1 == 0 && n2 == 0){
        result += 2.0 * interp::eval_df(z_eval, u_spline) * topo::bndlyr_dsc(r, lat, lon, 0);
        result += interp::eval_ddf(z_eval, u_spline) * topo::bndlyr_sc(r, lat, lon);
    } else if((n1 == 0 && n2 == 1) || (n1 == 1 && n2 == 0)) {
        result += interp::eval_df(z_eval, u_spline) * topo::bndlyr_dsc(r, lat, lon, 1);
    } else if((n1 == 0 && n2 == 2) || (n1 == 2 && n2 == 0)) {
        result += interp::eval_df(z_eval, u_spline) * topo::bndlyr_dsc(r, lat, lon, 2);
    }
...
double atmo::v(double r, double lat, double lon){
    double z_eval = interp::in_interval(r - globe::r0, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
    return interp::eval_f(z_eval, v_spline) * topo::bndlyr_sc(r, lat, lon);
}
double atmo::dv(double r, double lat, double lon, int n){
    double z_eval = interp::in_interval(r - globe::r0, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
    double result = interp::eval_f(z_eval, v_spline) * topo::bndlyr_dsc(r, lat, lon, n);
    
    if(n == 0){
        result += interp::eval_df(z_eval, v_spline) * topo::bndlyr_sc(r, lat, lon);
    }
    return result;
}
double atmo::ddv(double r, double lat, double lon, int n1, int n2){
    double z_eval = interp::in_interval(r - globe::r0, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
    double result = interp::eval_f(z_eval, v_spline) * topo::bndlyr_ddsc(r, lat, lon, n1, n2);
    
    if(n1 == 0 && n2 == 0){
        result += 2.0 * interp::eval_df(z_eval, v_spline) * topo::bndlyr_dsc(r, lat, lon, 0);
        result += interp::eval_ddf(z_eval, v_spline) * topo::bndlyr_sc(r, lat, lon);
    } else if((n1 == 0 && n2 == 1) || (n1 == 1 && n2 == 0)) {
        result += interp::eval_df(z_eval, v_spline) * topo::bndlyr_dsc(r, lat, lon, 1);
    } else if((n1 == 0 && n2 == 2) || (n1 == 2 && n2 == 0)) {
        result += interp::eval_df(z_eval, v_spline) * topo::bndlyr_dsc(r, lat, lon, 2);
    }
...
double atmo::w(double r, double lat, double lon){
    double z_eval, zg, X, S, dS[2], w0, sg_ratio, result = 0.0;

    if(geoac::is_topo && (r - globe::r0 < topo::z_bndlyr) && topo::use_BLw){
        z_eval = interp::in_interval(r - globe::r0, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
        ...
        dS[0] = -2.0 / topo::bndlyr_param * (X / pow(1.0 + X, 2)) * topo::dz(lat, lon, 0);
        dS[1] = -2.0 / topo::bndlyr_param * (X / pow(1.0 + X, 2)) * topo::dz(lat, lon, 1);

        w0  = interp::eval_f(z_eval, v_spline) * dS[0];
        w0 += interp::eval_f(z_eval, u_spline) * dS[1] / cos(lat);
...
double atmo::dw(double r, double lat, double lon, int n){
    double z_eval, X, dX[3], ddX[2], S, dS[3], ddS[2], N, D, sg_ratio, result = 0.0;
    ...
    if (geoac::is_topo && (r - globe::r0 < topo::z_bndlyr) && topo::use_BLw){
        ...
        u0 = interp::eval_f(z_eval, u_spline);
        v0 = interp::eval_f(z_eval, v_spline);
        ...
        dw0 = ddS[0] * v0 + ddS[1] * u0 / cos(lat);
        if (n == 0) {
            dw0 += interp::eval_df(z_eval, v_spline) * dS[1];
            dw0 += interp::eval_df(z_eval, u_spline) * dS[2] / cos(lat);
        } else if (n == 1){
...
double atmo::ddw(double r, double lat, double lon, int n1, int n2){
    double z_eval, zg, dzg[2], ddzg[2][2], result=0.0;
    double u0, v0, du0, dv0, X, dX[3], ddX[3][3], S, dS[3], ddS[3][3], N, D, dN[2], dD[2];
    double w0, dw0[2], sg_ratio, dddX, dddS[2], ddw0, ddN, ddD;

    if (geoac::is_topo && (r - globe::r0 < topo::z_bndlyr) && topo::use_BLw){
        // Compute z_eval, zg, and its derivatives
        z_eval = interp::in_interval(r - globe::r0, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
        zg = topo::z(lat, lon);
        for(int n = 0; n < 2; n++){
        ...
        v0 = interp::eval_f(z_eval, v_spline);
        if(n1 == 0 || n2 == 0){
            du0 = interp::eval_df(z_eval, u_spline);
            dv0 = interp::eval_df(z_eval, v_spline);
        } else {
            du0 = 0.0;
            dv0 = 0.0;
        }
        ...
        if (n1 == 0){
            dw0[0] += dv0 * dS[1] + du0 * dS[2] / cos(lat);
        } else if (n1 == 1){
        ...
        if(n2 == n1){
            ...
            if (n2 == 0){
                dw0[1] += dv0 * dS[1] + du0 * dS[2] / cos(lat);
            } else if (n2 == 1){
        ...
        if (n1 > 0 && n2 > 0){
            ...
            ddw0  = dddS[0] * v0 + ddS[n2][1] * dv0;
            ddw0 += (dddS[1] * u0 + ddS[n2][2] * du0) / cos(lat);;
            if (n2 == 1){
                ddw0 += sin(lat) / pow(cos(lat), 2) * (ddS[0][2] * u0 + dS[2] * du0);
            }
        } else if (n1 > 0 && n2 == 0){
            for(int k = 0; k < 2; k++){
            ...
            ddw0  = dddS[0] * v0 + ddS[n1][1] * dv0;
            ddw0 += (dddS[1] * u0 + ddS[n1][2] * du0) / cos(lat);;
            if (n1 == 1){
                ddw0 += sin(lat) / pow(cos(lat), 2) * (ddS[0][2] * u0 + dS[2] * du0);
            }
            ...
            ddw0  = dddS[0] * v0 + 2.0 * ddS[0][1] * dv0 + dS[1] * interp::eval_ddf(z_eval, v_spline);
            ddw0 += (dddS[1] * u0 + 2.0 * ddS[0][2] * du0 + dS[2] * interp::eval_ddf(z_eval, u_spline)) / cos(lat);
        }
...
void atmo::calc_uvw(double r, double lat, double lon, double & u, double & v, double & w, double du[3], double dv[3], double dw[3]){
    double z_eval, zg, dzg[2], ddzg[2][2], ddzg_temp[3];
    double u0, v0, du0[3], dv0[3], du_temp, ddu_temp;
    ...
    // Evaluate the u and v splines and first order derivatives
    interp::eval_all(z_eval, u_spline, u0, du_temp);
    for(int n = 0; n < 3; n++){
        du0[n] = 0.0;
    }
    du0[0] = du_temp;

    interp::eval_all(z_eval, v_spline, v0, du_temp);
    for(int n = 0; n < 3; n++){
        dv0[n] = 0.0;
    }
    dv0[0] = du_temp;
    ...
    for(int n = 0; n < 3; n++){
        if(n == 0){
            du0[n] = interp::eval_df(z_eval, u_spline);
            dv0[n] = interp::eval_df(z_eval, v_spline);
        } else {
            du0[n] = 0.0;
            dv0[n] = 0.0;
        }
...
void atmo::calc_uvw(double r, double lat, double lon, double & u, double & v, double & w, double du[3], double dv[3], double dw[3], double ddu[6], double ddv[6], double ddw[6]){
    double z_eval, zg, dzg[2], ddzg[2][2], dddzg[2][2][2], ddzg_temp [3], dddzg_temp [4];
    double u0, v0, du0[3], dv0[3], ddu0[3][3], ddv0[3][3], du_temp, ddu_temp;
    ...
    // Evaluate the u and v splines and first order derivatives
    interp::eval_all(z_eval, u_spline, u0, du_temp, ddu_temp);
    for(int n = 0; n < 3; n++){
        du0[n] = 0.0;
        for(int m = 0; m < 3; m++){
            ddu0[n][m] = 0.0; 
    ...
    interp::eval_all(z_eval, v_spline, v0, du_temp, ddu_temp);
    for(int n = 0; n < 3; n++){
        dv0[n] = 0.0;
        for(int m = 0; m < 3; m++){
            ddv0[n][m] = 0.0; 
    ...
    for(int n = 0; n < 3; n++){
        ...
        ddu[n] = ddu0[n][n] * sc0 + 2.0 * du0[n] * dsc0[n] + u0 * ddsc0[n][n];
        ddv[n] = ddv0[n][n] * sc0 + 2.0 * dv0[n] * dsc0[n] + v0 * ddsc0[n][n];
    }
    ddu[3] = ddu0[0][1] * sc0 + du0[0] * dsc0[1] + du0[1] * dsc0[0] + u0 * ddsc0[0][1];
    ddu[4] = ddu0[0][2] * sc0 + du0[0] * dsc0[2] + du0[2] * dsc0[0] + u0 * ddsc0[0][2];
    ddu[5] = ddu0[1][2] * sc0 + du0[1] * dsc0[2] + du0[2] * dsc0[1] + u0 * ddsc0[1][2];
    
    ddv[3] = ddv0[0][1] * sc0 + dv0[0] * dsc0[1] + dv0[1] * dsc0[0] + v0 * ddsc0[0][1];
    ddv[4] = ddv0[0][2] * sc0 + dv0[0] * dsc0[2] + dv0[2] * dsc0[0] + v0 * ddsc0[0][2];
    ddv[5] = ddv0[1][2] * sc0 + dv0[1] * dsc0[2] + dv0[2] * dsc0[1] + v0 * ddsc0[1][2];
    ...
    if (geoac::is_topo && (r - globe::r0 < topo::z_bndlyr) && topo::use_BLw){
        ...
        for(int n = 0; n < 3; n++){
            ...
            if(n == 0){
                ddw0 += 2.0 * ddsc0[1][0] * dv0[0] + dsc0[1] * ddv0[0][0];
                ddw0 += (2.0 * ddsc0[2][0] * du0[0] + dsc0[2] * ddu0[0][0]) / cos(lat);
            } else if(n == 1){
        ...
        // ddw[3-5] are the drdlat, drdlon, and dlatdlon derivatives
...
#endif /* _ATMO_STATE_SPH_STRAT_CPP_ */
