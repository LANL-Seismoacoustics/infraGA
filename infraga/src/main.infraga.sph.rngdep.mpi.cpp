...
void run_prop(char* inputs[], int count){
    ...
    if (geoac::is_topo){    file_check = set_region(inputs[2], inputs[3], inputs[4], topo_file, prof_format, reverse_winds, world_rank);}
    else{                   file_check = set_region(inputs[2], inputs[3], inputs[4], prof_format, reverse_winds, world_rank);}
    if(!file_check){
        MPI_Finalize();
        return;
    }
...
void run_mach_cone(char* inputs[], int count){
    ...
    // Define variables used for analysis
	double lambda_step, D, D_prev, travel_time_sum, attenuation, r_max, inclination, GC1, GC2, back_az;
	int k, length = int(geoac::s_max / geoac::ds_min);
	bool break_check;
