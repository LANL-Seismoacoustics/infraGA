#ifndef _GEOAC_EQSET_SPH_STRAT_CPP_
#define _GEOAC_EQSET_SPH_STRAT_CPP_
...
void geoac::set_refl(double** & solution, int k_end){
    ...
    if(calc_amp){
        ...
        dnu_th_ds += (pow(c_grnd, 2) / refs.c0) * (prev[3] * prev[4] + pow(prev[5], 2) * tan(prev[1]));
...
void geoac::update_refs(double ray_length, double* current_values){
    ...
    double dd_c[3][3], dd_u[3][3], dd_v[3][3], dd_w[3][3], dc_temp, ddc_temp;
    double R_lt[3], R_lp[3], mu_lt[3], mu_lp[3];

    refs.nu_mag = 	 sqrt(pow(nu[0],2) + pow(nu[1],2) + pow(nu[2], 2));
    
    if(!calc_amp){
        double z_eval = interp::in_interval(current_values[0] - globe::r0, atmo::c_spline.x_vals[0], atmo::c_spline.x_vals[atmo::c_spline.length - 1]);
        ...
        */
        ...
        for(int m = 0; m < 3; m++){
            refs.ddc[m][0] = 0.0;   refs.ddw[m][0] = 0.0;   refs.ddv[m][0] = 0.0;   refs.ddu[m][0] = 0.0;
            refs.ddc[m][1] = 0.0;   refs.ddw[m][1] = 0.0;   refs.ddv[m][1] = 0.0;   refs.ddu[m][1] = 0.0;
        }
        ...
        // Update geometry term derivatives
        ...
        // refs.T[0] = - 1.0 / r * (nu[1] * refs.c_gr[1] + nu[2] * refs.c_gr[2]);
        ...
        // refs.T[1] = (nu[0] * refs.v - nu[1] * refs.w) + nu[0] * refs.c_gr[1] + nu[2] * refs.c_gr[2] * tan(t);
        refs.dT[1][0] = (mu_lt[0] * refs.v + nu[0] * refs.dv[3] - mu_lt[1] * refs.w - nu[1] * refs.dw[3]) + mu_lt[0] * refs.c_gr[1] + nu[0] * refs.dc_gr[1][0];
        refs.dT[1][1] = (mu_lp[0] * refs.v + nu[0] * refs.dv[4] - mu_lp[1] * refs.w - nu[1] * refs.dw[4]) + mu_lp[0] * refs.c_gr[1] + nu[0] * refs.dc_gr[1][1];

        refs.dT[1][0] += (mu_lt[2] * refs.c_gr[2] + nu[2] * refs.dc_gr[2][0]) * tan(t) + nu[2] * refs.c_gr[2] * R_lt[1] / pow(cos(t), 2);
        refs.dT[1][1] += (mu_lp[2] * refs.c_gr[2] + nu[2] * refs.dc_gr[2][1]) * tan(t) + nu[2] * refs.c_gr[2] * R_lp[1] / pow(cos(t), 2);

        // refs.T[2] = (nu[0] * refs.u - nu[2] * refs.w) * cos(t) + (nu[1] * refs.u - nu[2] * refs.v) * sin(t) + refs.c_gr[2] * (nu[0] * cos(t) - nu[1] * sin(t));
...
//-------------------------------------------------------//
//-------Check if ray has left propagation region,-------//
//----------returned to ground, reached a pole,----------//
//----------or longitudes beyond +/-180 degrees----------//
//-------------------------------------------------------//
bool geoac::break_check(double ** & solution, int k){
    ...
    if(solution[k][1] < -Pi / 2.0 || solution[k][1] > Pi / 2.0){
        cout << "WARNING - Ray reached pole.  Division by zero in coordinate transform..." << '\n';
        return true;
    }
...
#endif /* _GEOAC_EQSET_SPH_STRAT_CPP_ */
