...
void run_prop(char* inputs[], int count){
    ...
    bool write_atmo=false, write_caustics=false, custom_output_id=false, print_resid=false, reverse_winds=false;
...
void run_wnl_wvfrm(char* inputs[], int count){
    ...
    bool break_check, write_atmo=false, write_rays=false, custom_output_id=false, start_wvfrm=false, reverse_winds=false;
    char* prof_format = "zTuvdp";
    char* topo_file = "none";
    char input_check;
        ...
        if(!start_wvfrm){
            for(wvfrm_ref_k = 0; wvfrm_ref_k < k; wvfrm_ref_k++){
                ...
                if (ray_length >= wvfrm_ref){
                    ...
                    ray_length = geoac::wnl_wvfrm(solution, wvfrm_array, wvfrm_ref_k + 1, k, wvfrm_ref, c0, rho0, D0, p0, wvfrm_out_step);
                    break;
