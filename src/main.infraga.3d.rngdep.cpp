...
void usage(){
    cout << '\n';
    cout << '\t' << "#################################################" << '\n';
    cout << '\t' << "####            infraga-3d-rngdep            ####" << '\n';
    cout << '\t' << "####      Three-Dimensional Ray Tracing      ####" << '\n';
    cout << '\t' << "#### Through an Inhomogeneous, Moving Medium ####" << '\n';
    cout << '\t' << "#################################################" << '\n' << '\n';
    
    cout << '\n';
    cout << "Usage: infraga-3d-rngdep [option] profile_id nodes-x.loc nodes-y.loc [parameters]" << '\n';
    ...
    cout << '\t' << '\t' << '\t' << "Profiles are ordered such that profile_id[N].met describes the atmosphere at x = x_i, y = y_j such that N = i + n_x * j" << '\n';
    ...
    cout << '\t' << '\t' << "az_est"            << '\t' << '\t' << "degrees"    << '\t' << '\t' << "atan2(dy, dx)" << '\n';
    cout << '\t' << '\t' << "bounces"           << '\t' << '\t' << "integer"    << '\t' << '\t' << "0" << '\n';
    cout << '\t' << '\t' << "src_x"             << '\t' << '\t' << "km"         << '\t' << '\t' << "midpoint of loc-x file" << '\n';
    cout << '\t' << '\t' << "src_y"             << '\t' << '\t' << "km"         << '\t' << '\t' << "midpoint of loc-y file" << '\n';
    cout << '\t' << '\t' << "src_alt"           << '\t' << '\t' << "km"         << '\t' << '\t' << "0.0" << '\n';
...
void run_prop(char* inputs[], int count){
    cout << '\n';
    cout << '\t' << "#########################################" << '\n';
    cout << '\t' << "####    Running infraga-3d-rngdep    ####" << '\n';
    cout << '\t' << "####           Propagation           ####" << '\n';
    cout << '\t' << "#########################################" << '\n' << '\n';
    ...
    results << "# infraga-3d-rngdep -prop summary:" << '\n';
...
void run_back_proj(char* inputs[], int count){
    ...
    sprintf(output_buffer, "%s.projection.dat", file_id);
...
void run_eig_search(char* inputs[], int count){
    ...
    src[2] = max(src[2], topo::z(src[0], src[1]));
    ...
    geoac::eig_results << "# infraga-3d-rngdep -eig_search summary:" << '\n';
    ...
    for(int n_bnc = bnc_min; n_bnc <= bnc_max; n_bnc++){
        cout << "Searching for " << n_bnc << " bounce eigenray(s) between " << theta_min * 180.0 / Pi << " and " << theta_max * 180.0 / Pi << "." << '\n';
...
void run_eig_direct(char* inputs[], int count){
    ...
    for(int i = 5; i < count; i++){ if (strncmp(inputs[i], "phi_est=", 8) == 0){ phi_est = Pi / 2.0 - atof(inputs[i] + 8);}}
    ...
    cout << '\t' << "azimuth estimate: " << phi_est  * (180.0 / Pi)<< '\n';
    cout << '\t' << "bounces: " << bounces << '\n';
    cout << '\t' << "damping:" << geoac::damping << '\n';
    cout << '\t' << "frequency: " << freq << '\n';
    cout << '\t' << "S&B atten coeff: " << atmo::tweak_abs << '\n' << '\n';

    geoac::find_eigenray(src, rcvr, theta_est, phi_est, freq, bounces, iterations, inputs[2]);
    clear_region();
}
...
void run_wnl_wvfrm(char* inputs[], int count){
    ...
    wvfrm_out << "# infraga-3d-rngdep -wnl_wvfrm summary:" << '\n';
    wvfrm_out << "#" << '\t' << "profile prefix: " << inputs[2] << '\n';
    wvfrm_out << "#" << '\t' << "profile grid files: " << inputs[3] << ", " << inputs[4] << '\n';
...
void run_region_test(char* inputs[], int count){
    ...
    cout << '\n' << "Running region test comparing defined derivatives to finite differences (make sure you've created 'region_tests/3D_rngdep/' directory)..." << '\n';
    cout << '\t' << "Evaluating sound speed..." << '\n';
    file_out.open("region_tests/3D_rngdep/c.analytic.dat");
    for(int m = 0; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/c.finite.dat");
    ...
    file_out.open("region_tests/3D_rngdep/u.analytic.dat");
    for(int m = 1; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/u.finite.dat");
    for(int m = 1; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/u.calc_uvw.dat");
    for(int m = 0; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/v.analytic.dat");
    for(int m = 0; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/v.finite.dat");
    for(int m = 1; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/v.calc_uvw.dat");
    for(int m = 0; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/w.analytic.dat");
    for(int m = 0; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/w.finite.dat");
    for(int m = 1; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    file_out.open("region_tests/3D_rngdep/w.calc_uvw.dat");
    for(int m = 0; m < int(alt_max * 100); m++){
        double z0 = topo::z(x_src, y_src) + m/100.0;
        if(z0 > atmo::c_spline.z_vals[atmo::c_spline.length_z - 1]) break;
    ...
    if(geoac::is_topo){
        ...
        cout << '\t' << '\t' << "field..." << '\n';
        file_out.open("region_tests/3D_rngdep/topo.dat");
        ...
        cout << '\t' << '\t' << "analytic..." << '\n';
        file_out.open("region_tests/3D_rngdep/topo.x.analytic.dat");
        ...
        file_out.open("region_tests/3D_rngdep/topo.y.analytic.dat");
        ...
        cout << '\t' << '\t' << "finite diff..." << '\n';        
        file_out.open("region_tests/3D_rngdep/topo.x.finite.dat");
        for(int m = 0; m < 2 * topo::spline.length_x; m++){
            double x0 = topo::spline.x_vals[0] + m / (2.0 * topo::spline.length_x) * (topo::spline.x_vals[topo::spline.length_x - 1] - topo::spline.x_vals[0]);
            double y0 = y_src;
        ...
        file_out.open("region_tests/3D_rngdep/topo.y.finite.dat");
        for(int m = 0; m < 2 * topo::spline.length_y; m++){
            double x0 = x_src;
            double y0 = topo::spline.y_vals[0] + m / (2.0 * topo::spline.length_y) * (topo::spline.y_vals[topo::spline.length_y - 1] - topo::spline.y_vals[0]);
        ...
        cout << '\t' << '\t' << "calc_all..." << '\n';
        file_out.open("region_tests/3D_rngdep/topo.x.calc_all.dat");
        for(int m = 0; m < 2 * topo::spline.length_x; m++){
            double x0 = topo::spline.x_vals[0] + m / (2.0 * topo::spline.length_x) * (topo::spline.x_vals[topo::spline.length_x - 1] - topo::spline.x_vals[0]);
            double y0 = y_src;
        ...
        file_out.open("region_tests/3D_rngdep/topo.y.calc_all.dat");
        for(int m = 0; m < 2 * topo::spline.length_y; m++){
            double x0 = x_src;
            double y0 = topo::spline.y_vals[0] + m / (2.0 * topo::spline.length_y) * (topo::spline.y_vals[topo::spline.length_y - 1] - topo::spline.y_vals[0]);
