# ifndef _ATMO_STATE_3D_STRAT_CPP_
# define _ATMO_STATE_3D_STRAT_CPP_
...
double topo::z(double x, double y){
    double result = z0;
    if (geoac::is_topo){
        result = interp::eval_f(interp::in_interval(x, spline.x_vals[0], spline.x_vals[spline.length_x - 1]),
...
double topo::ddz(double x, double y, int n1, int n2){
    double result = 0.0;
    if (geoac::is_topo){
        return interp::eval_ddf(interp::in_interval(x, spline.x_vals[0], spline.x_vals[spline.length_x - 1]),
...
double topo::bndlyr_ddsc(double x_val, double y_val, double z_val, int n1, int n2){
    ...
    if (z_val < z_bndlyr){
        ...
        if(n1 < 2 && n2 < 2){
            ddX = X / bndlyr_param * (ddz(x_val, y_val, n1, n2) + 1.0 / bndlyr_param * dz(x_val, y_val, n1) * dz(x_val, y_val, n2));
        } else if(n1 < 2 && n2 == 2){
...
double atmo::u(double x, double y, double z){
    ...
    return interp::eval_f(z_eval, u_spline) * topo::bndlyr_sc(x, y, z);
}

double atmo::du(double x, double y, double z, int n){
    double z_eval = interp::in_interval(z, u_spline.x_vals[0], u_spline.x_vals[u_spline.length - 1]);
    double result = interp::eval_f(z_eval, u_spline) * topo::bndlyr_dsc(x, y, z, n);

    if(n == 2){       
        result += interp::eval_df(z_eval, u_spline) * topo::bndlyr_sc(x, y, z);
    }
    return result;
}
...
double atmo::ddu(double x, double y, double z, int n1, int n2){
    double z_eval = interp::in_interval(z, u_spline.x_vals[0], u_spline.x_vals[u_spline.length - 1]);
    double result = interp::eval_f(z_eval, u_spline) * topo::bndlyr_ddsc(x, y, z, n1, n2);

    if(n1 < 2 && n2 == 2){
        result += interp::eval_df(z_eval, u_spline) * topo::bndlyr_dsc(x, y, z, n1);
    } else if(n1 == 2 && n2 < 2){
        result += interp::eval_df(z_eval, u_spline) * topo::bndlyr_dsc(x, y, z, n2);
    } else if (n1 == 2 && n2 == 2) {
        result += interp::eval_ddf(z_eval, u_spline) * topo::bndlyr_sc(x, y, z);
        result += 2.0 * interp::eval_df(z_eval, u_spline) * topo::bndlyr_dsc(x, y, z, 2);
    }
    return result;
}

double atmo::v(double x, double y, double z){
    double z_eval = interp::in_interval(z, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
    return interp::eval_f(z_eval, v_spline) * topo::bndlyr_sc(x,y,z);
}
double atmo::dv(double x, double y, double z, int n){
    double z_eval = interp::in_interval(z, v_spline.x_vals[0], v_spline.x_vals[v_spline.length - 1]);
    double result = interp::eval_f(z_eval, v_spline) * topo::bndlyr_dsc(x, y, z, n);

    if(n == 2){       
        result += interp::eval_df(z_eval, v_spline) * topo::bndlyr_sc(x, y, z);
    }
    return result;
}

double atmo::ddv(double x, double y, double z, int n1, int n2){
    double z_eval = interp::in_interval(z, v_spline.x_vals[0], u_spline.x_vals[v_spline.length - 1]);
    double result = interp::eval_f(z_eval, v_spline) * topo::bndlyr_ddsc(x, y, z, n1, n2);

    if(n1 < 2 && n2 == 2){
        result += interp::eval_df(z_eval, v_spline) * topo::bndlyr_dsc(x, y, z, n1);
    } else if(n1 == 2 && n2 < 2){
        result += interp::eval_df(z_eval, v_spline) * topo::bndlyr_dsc(x, y, z, n2);
    } else if (n1 == 2 && n2 == 2) {
        result += interp::eval_ddf(z_eval, v_spline) * topo::bndlyr_sc(x, y, z);
        result += 2.0 * interp::eval_df(z_eval, v_spline) * topo::bndlyr_dsc(x, y, z, 2);
    }
    return result;
}

double atmo::w(double x, double y, double z){
    double z_eval, X, S, dS[2], w0, sg_ratio, result = 0.0;
    ...
    if (geoac::is_topo && (z < topo::z_bndlyr) && topo::use_BLw){
        ...
        w0  = interp::eval_f(z_eval, u_spline) * dS[0];
        w0 += interp::eval_f(z_eval, v_spline) * dS[1];
...
double atmo::dw(double x, double y, double z, int n){
    ...
    if (geoac::is_topo && (z < topo::z_bndlyr) && topo::use_BLw){
        ...
        if(n == 2){
            dw0 += interp::eval_df(z_eval, u_spline) * dS[0];
...
double atmo::ddw(double x, double y, double z, int n1, int n2){
    double z_eval, X, S, N, D, sg_ratio, result = 0.0;
    ...
    if (geoac::is_topo && (z < topo::z_bndlyr) && topo::use_BLw){
        ...
        if (n1 < 2 && n2 < 2){
            ...
            ddw0  = dddS[0] * u0 + 2.0 * ddS[0][2] * du0 + dS[0] * interp::eval_ddf(z_eval, u_spline);
            ddw0 += dddS[1] * v0 + 2.0 * ddS[1][2] * dv0 + dS[1] * interp::eval_ddf(z_eval, v_spline);
        }
...
void atmo::calc_uvw(double x, double y, double z, double & u, double & v, double & w, double du[3], double dv[3], double dw[3]){
    double z_eval, zg, dzg[2], ddzg_temp[3], ddzg[2][2];
    double u0, v0, du_temp, du0[3], dv0[3];
    ...
    interp::eval_all(z_eval, atmo::u_spline, u0, du_temp);
    for(int n = 0; n < 2; n++){
        du0[n] = 0.0;
    }
    du0[2] = du_temp;

    interp::eval_all(z_eval, atmo::v_spline, v0, du_temp);
    for(int n = 0; n < 2; n++){
        dv0[n] = 0.0;
    }
    dv0[2] = du_temp;
    ...
    if (geoac::is_topo && (z < topo::z_bndlyr) && topo::use_BLw){
        ...
        for(int n = 0; n < 3; n++){
            dw0 = u0 * ddsc0[0][n] + v0 * ddsc0[1][n];
            dw0 += du0[n] * dsc0[0] + dv0[n] * dsc0[1];
            dw0 *= -2.0 * topo::bndlyr_param;
...
void atmo::calc_uvw(double x, double y, double z, double & u, double & v, double & w, double du[3], double dv[3], double dw[3], double ddu[6], double ddv[6], double ddw[6]){
    ...
    for(int n = 0; n < 2; n++){
        ...
        for(int m = n; m < 3; m++){
            if(m < 2){
                ddX[n][m] = X / topo::bndlyr_param * (ddzg[n][m] + 1.0 / topo::bndlyr_param * dzg[n] * dzg[m]);
                for(int k = 0; k < 2; k++){
    ...
    if (geoac::is_topo && (z < topo::z_bndlyr) && topo::use_BLw){
        ...
        for(int n = 0; n < 3; n++){
            dw0[n] = u0 * ddsc0[0][n] + v0 * ddsc0[1][n];
            dw0[n] += du0[n] * dsc0[0] + dv0[n] * dsc0[1];
            dw0[n] *= -2.0 * topo::bndlyr_param;
            ...
            if(n < 2){
                ...
                ddw0  = dddsc0[0][2][2] * u0 + 2.0 * ddsc0[0][2] * du0[2] + dsc0[0] * ddu0[2][2];
                ddw0 += dddsc0[1][2][2] * v0 + 2.0 * ddsc0[1][2] * dv0[2] + dsc0[1] * ddv0[2][2];
                ddw0 *= -2.0 * topo::bndlyr_param;
            }

            ddN  = ddsc0[n][n] * X * w0 + sc0 * ddX[n][n] * w0 + sc0 * X * ddw0;
            ddN += 2.0 * (dsc0[n] * dX[n] * w0 + dsc0[n] * X * dw0[n] + sc0 * dX[n] * dw0[n]);
            ddD = (1.0 - sg_ratio * X) * ddX[n][n] - sg_ratio * pow(dX[n], 2);

            ddw[n] = ((ddN * D - 2.0 * dN[n] * dD[n] - N * ddD) * pow(D, 2) + 2.0 * N * D * pow(dD[n], 2)) / pow(D, 4);
        }

        // ddw[3-5] are the dxdy, dxdz, and dydz derivatives
        ...
        ddD = (1.0 - sg_ratio * X) * ddX[0][1] - sg_ratio * dX[0] * dX[1];
        ...
        ddD = (1.0 - sg_ratio * X) * ddX[0][2] - sg_ratio * dX[0] * dX[2];
        ...
        ddD = (1.0 - sg_ratio * X) * ddX[1][2]- sg_ratio * dX[1] * dX[2];
...
#endif /* _ATMO_STATE_3D_STRAT_CPP_ */
