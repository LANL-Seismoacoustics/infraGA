...
void usage(){
    cout << '\n';
    cout << '\t' << "#########################################################" << '\n';
    cout << '\t' << "####                 infraga-accel-3d                ####" << '\n';
    cout << '\t' << "####    Accelerated Three-Dimensional Ray Tracing    ####" << '\n';
    cout << '\t' << "####       Through a Stratified, Moving Medium       ####" << '\n';
    cout << '\t' << "#########################################################" << '\n' << '\n';
    
    cout << '\n';
    cout << "Usage: mpirun -np [number] infraga-accel-3d [option] profile.met [parameters]" << '\n';
    ...
    cout << "Examples:" << '\n';
    cout << '\t' << "mpirun -np 6 ./bin/infraga-accel-3d -prop examples/ToyAtmo.met incl_step=1.0 azimuth=-45.0 write_rays=true" << '\n';
    cout << '\t' << "mpirun -np 6 ./bin/infraga-accel-3d -prop examples/ToyAtmo.met bounces=0 az_min=-120.0 az_max=-60.0 az_step=3.0" << '\n';
    cout << '\t' << "mpirun -np 6 ./bin/infraga-accel-3d -eig_search examples/ToyAtmo.met rcvr_x=-175.0 rcvr_y=75.0 verbose=true incl_step_max=0.2" << '\n' << '\n';
}
...
void run_prop(char* inputs[], int count){
    ...
    bool  write_atmo = false, write_rays = false, write_topo=false, custom_output_id=false, reverse_winds=false;
    ...
    if (geoac::is_topo){    file_check = set_region(inputs[2], topo_file, prof_format, reverse_winds, world_rank);}
    else {                  file_check = set_region(inputs[2], prof_format, reverse_winds, world_rank);}
    if (!file_check){
        MPI_Finalize();
        return;
    }
    ...
    if(world_rank==0){
        ...
        results << "# infraga-accel-3d -prop summary:" << '\n';
    ...
    for(double phi = phi_min;       phi <= phi_max;     phi += phi_step){
        ...
        for(double theta = max(theta_min, theta_grnd); theta <= theta_max; theta+=theta_step * world_size){
...
void run_eig_search(char* inputs[], int count){
    ...
    bool sequential_srcs = true, write_atmo = false, custom_output_id=false, reverse_winds=false;
    
    double src0 [3]  = {0.0, 0.0, 0.0};
    double rcvr0 [2] = {200.0, 0.0};
    ...
    if (strncmp(rcvrs_file, "None", 4) == 0){
        ...
        if(world_rank == 0){
            rcvrs_cnt = file_length(rcvrs_file);
        }
    ...
    for(int n_src = 0; n_src < srcs_cnt; n_src++){
        ...
        if(group_rank == 0){
            ...
            geoac::eig_results << "# infraga-accel-3d -eig_search summary:" << '\n';
        ...
        for(int n_rcvr = 0; n_rcvr < rcvrs_cnt; n_rcvr++){
            if(task_id == n_rcvr){
                if(seq_srcs){
                    seq_dr = sqrt(pow(srcs[n_src][0] - seq_srcs_ref[0] , 2)
                                    + pow(srcs[n_src][1] - seq_srcs_ref[1] , 2)
                                        + pow(srcs[n_src][2] - seq_srcs_ref[2] , 2));
