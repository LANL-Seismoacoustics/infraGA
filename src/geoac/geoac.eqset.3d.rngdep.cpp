...
namespace geoac {
    struct src_refs{
        ...
        double c_gr[3]; double c_gr_mag; double dc_gr[3][2]; double dc_gr_mag[2];   // Group velocity components, magnitude, and variattions of each
    };
...
void geoac::set_refl(double** & solution, int k){
    ...
    if(calc_amp){
        ...
        ds0_dth = - refs.c0 / c_grnd * (prev[8] - a * prev[6] - b * prev[7]) / norm2;                           ds0_dph = - refs.c0 / c_grnd * (prev[14] - a * prev[12] - b * prev[13]) / norm2;
...
void geoac::update_refs(double ray_length, double* current_values){
    ...
    if(!calc_amp){
        ...
        refs.c_gr[0] =  refs.c * nu[0] / refs.nu_mag + refs.u;
        refs.c_gr[1] =  refs.c * nu[1] / refs.nu_mag + refs.v;
        refs.c_gr[2] =  refs.c * nu[2] / refs.nu_mag + refs.w;
        ...
        X_th[0]  = current_values[6];   mu_th[0] = current_values[9];   X_ph[0]  = current_values[12];  mu_ph[0] = current_values[15];
        X_th[1]  = current_values[7];   mu_th[1] = current_values[10];  X_ph[1]  = current_values[13];  mu_ph[1] = current_values[16];
        X_th[2]  = current_values[8];   mu_th[2] = current_values[11];  X_ph[2]  = current_values[14];  mu_ph[2] = current_values[17];

        refs.dc[3] = 0.0;      refs.dc[4] = 0.0;
        refs.du[3] = 0.0;      refs.du[4] = 0.0;
        refs.dv[3] = 0.0;      refs.dv[4] = 0.0;
        refs.dw[3] = 0.0;      refs.dw[4] = 0.0;
        for(int n = 0; n < 3; n++){
            ...
            refs.ddc[n][0] = 0.0; refs.ddu[n][0] = 0.0; refs.ddv[n][0] = 0.0; refs.ddw[n][0] = 0.0;
            refs.ddc[n][1] = 0.0; refs.ddu[n][1] = 0.0; refs.ddv[n][1] = 0.0; refs.ddw[n][1] = 0.0;
            ...
            for(int m = 0; m < 3; m++){
                ...
                refs.ddw[n][0] += X_th[m] * atmo::ddw(x, y, z, n, m);
                refs.ddw[n][1] += X_ph[m] * atmo::ddw(x, y, z, n, m);
            }
        }
        ...
        for (int n = 0; n < 3; n++){
            refs.dc_gr[n][0] = nu[n] / refs.nu_mag * refs.dc[3] + refs.c * mu_th[n] / refs.nu_mag - refs.c * nu[n] / pow(refs.nu_mag,2) * refs.dnu_mag[0];
            refs.dc_gr[n][1] = nu[n] / refs.nu_mag * refs.dc[4] + refs.c * mu_ph[n] / refs.nu_mag - refs.c * nu[n] / pow(refs.nu_mag,2) * refs.dnu_mag[1];
        }
            
        refs.dc_gr[0][0] += refs.du[3];   refs.dc_gr[0][1] += refs.du[4];
        refs.dc_gr[1][0] += refs.dv[3];   refs.dc_gr[1][1] += refs.dv[4];
        refs.dc_gr[2][0] += refs.dw[3];   refs.dc_gr[2][1] += refs.dw[4];
...
double geoac::eval_src_eq(double ray_length, double* current_values, int n_eq){
    ...
    nu[0] = current_values[3];
    nu[1] = current_values[4];
    nu[2] = current_values[5];
    ...
    switch(n_eq){
        ...
        case(1):    // d y / d s
        case(2):    // d z / d s
            result = refs.c_gr[n_eq] / refs.c_gr_mag;
            break;
            
        case(3): 	// d nu_x /ds
        case(4): 	// d nu_y / ds
        case(5): 	// d nu_z / ds
            result = -1.0 / refs.c_gr_mag * (refs.nu_mag * refs.dc[n_eq - 3] + nu[0] * refs.du[n_eq - 3] + nu[1] * refs.dv[n_eq - 3] + nu[2] * refs.dw[n_eq - 3]);
            break;
            
        case(6):	// d X_th/ds
        case(7):	// d Y_th/ds
        case(8): 	// d Z_th/ds
            result = refs.dc_gr[n_eq - 6][0] / refs.c_gr_mag - refs.c_gr[n_eq - 6] / pow(refs.c_gr_mag, 2) * refs.dc_gr_mag[0];
            break;
            
        case(9): 	// d mu_x_th /ds
        case(10): 	// d mu_y_th /ds
        case(11): 	// d mu_z_th /ds
            result = 1.0 / pow(refs.c_gr_mag, 2) * refs.dc_gr_mag[0] * (refs.nu_mag * refs.dc[n_eq - 9] + nu[0] * refs.du[n_eq - 9] + nu[1] * refs.dv[n_eq - 9] + nu[2] * refs.dw[n_eq - 9])
                                        - 1.0 / refs.c_gr_mag * (refs.dnu_mag[0] * refs.dc[n_eq - 9] + refs.nu_mag * refs.ddc[n_eq - 9][0]
                                                  + mu_th[0] * refs.du[n_eq - 9] + mu_th[1] * refs.dv[n_eq - 9] + mu_th[2] * refs.dw[n_eq - 9]
        ...
        case(13):  	// d Y_ph/ds
        case(14):	// d Z_ph/ds
            result = refs.dc_gr[n_eq - 12][1] / refs.c_gr_mag - refs.c_gr[n_eq - 12] / pow(refs.c_gr_mag, 2) * refs.dc_gr_mag[1];
            break;
            
        case(15): 	// d mu_x_ph/ds
        case(16): 	// d mu_y_ph/ds
        case(17): 	// d mu_z_ph/ds
            result =  1.0 / pow(refs.c_gr_mag, 2) * refs.dc_gr_mag[1] * (refs.nu_mag * refs.dc[n_eq - 15] + nu[0] * refs.du[n_eq - 15] + nu[1] * refs.dv[n_eq - 15] + nu[2] * refs.dw[n_eq - 15])
                                        - 1.0 / refs.c_gr_mag * (refs.dnu_mag[1] * refs.dc[n_eq - 15] + refs.nu_mag * refs.ddc[n_eq - 15][1]
                                                  + mu_ph[0] * refs.du[n_eq - 15] + mu_ph[1] * refs.dv[n_eq - 15] + mu_ph[2] * refs.dw[n_eq - 15]
...
bool geoac::ground_check(double ** solution, int k){
    if (solution[k][2] < topo::z_max){ if(solution[k][2] < topo::z(solution[k][0], solution[k][1])){ return true;}}
    return false;
}
...
double geoac::travel_time(double ** solution, int k){
	...
	for (int n = 0; n < k; n++){
        ...
        sndspd = atmo::c(x, y, z);
		c_prop[0] = sndspd * nu[0] / nu_mag + atmo::u(x, y, z);
		c_prop[1] = sndspd * nu[1] / nu_mag + atmo::v(x, y, z);
		c_prop[2] = sndspd * nu[2] / nu_mag + atmo::w(x, y, z);
		c_prop_mag = sqrt(pow(c_prop[0], 2) + pow(c_prop[1], 2) + pow(c_prop[2], 2));
        
		traveltime += ds/c_prop_mag;
    }
	return traveltime;
}
...
double geoac::jacobian(double ** solution, int k){
    double sndspd, nu_mag, c_prop[3], c_prop_mag;
	...
	c_prop[0] = sndspd * solution[k][3] / nu_mag + atmo::u(solution[k][0], solution[k][1], solution[k][2]);
    c_prop[1] = sndspd * solution[k][4] / nu_mag + atmo::v(solution[k][0], solution[k][1], solution[k][2]);
    c_prop[2] = sndspd * solution[k][5] / nu_mag + atmo::w(solution[k][0], solution[k][1], solution[k][2]);
	c_prop_mag = sqrt(pow(c_prop[0], 2) + pow(c_prop[1], 2) + pow(c_prop[2], 2));
    
    dxds = c_prop[0] / c_prop_mag;  dyds = c_prop[1] / c_prop_mag;  dzds = c_prop[2] / c_prop_mag;
	dxdth = solution[k][6];         dydth = solution[k][7];         dzdth = solution[k][8];
	dxdph = solution[k][12];        dydph = solution[k][13];        dzdph = solution[k][14];
    
	return	dxds*(dydth*dzdph - dydph*dzdth) - dxdth*(dyds*dzdph - dzds*dydph) + dxdph*(dyds*dzdth - dzds*dydth);
}

double geoac::amp(double ** solution, int k){
    double sndspd, winds[3], winds0[3], nu_mag, nu_mag0, c_prop[3], c_prop0[3], c_prop_mag, c_prop_mag0, num, den;
    ...
    for (int n = 0; n < 3; n++){ c_prop[n] =  sndspd * solution[k][3 + n] / nu_mag + winds[n];}
    c_prop0[0] = refs.c0 * cos(theta) * cos(phi) + winds0[0];
    c_prop0[1] = refs.c0 * cos(theta) * sin(phi) + winds0[1];
    c_prop0[2] = refs.c0 * sin(theta) + winds0[2];
    
	c_prop_mag = sqrt(pow(c_prop[0], 2) + pow(c_prop[1], 2) + pow(c_prop[2], 2));
    c_prop_mag0 = sqrt(pow(c_prop0[0], 2) + pow(c_prop0[1], 2) + pow(c_prop0[2], 2));
	
	num = atmo::rho(solution[k][0], solution[k][1], solution[k][2]) * nu_mag * pow(sndspd, 3) * c_prop_mag0 * cos(theta);
	den = atmo::rho(refs.src_loc[0], refs.src_loc[1], refs.src_loc[2]) * nu_mag0* pow(refs.c0,3) * c_prop_mag * jacobian(solution, k);
...
double geoac::wnl_wvfrm(double** solution, double** & u, int k1, int k2, double s_prev, double c0, double cg0, double nu0, double rho0, double D0, double p0, double output_step){
    ...
    while(s < s_tot){
        ...
        if (fabs(10.0 * ceil((s + s_prev + ds) / 10.0) - 10.0 * ceil((s + s_prev) / 10.0)) > 0.0){
            cout << '\t' << "Generating waveform at ray length: " << 10.0 * ceil((s + s_prev) / 10.0) << '\t' << "altitude: " << interp::eval_f(s, ray_z) << '\t' << "scaled non-linearity factor: " << beta_curr << '\n';
        }
