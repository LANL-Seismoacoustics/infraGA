...
void geoac::find_eigenray(double src[3], double rcvr[2], double th_est, double ph_est, double freq, int bnc_cnt, int iterate_limit, char title[]){
	...
	for(int n = 0; n <= iterate_limit; n++){
        ...
        if(verbose) cout << '\t' << '\t' << "Arrival after " << bnc_cnt << " reflections at (" << x << ", " << y << "), distance to receiver = " << dr << " km." << '\n';
        ...
        if(dr < tolerance){
            ...
            if(sqrt(pow(dth, 2) + pow(dph, 2)) * step_sc < 1.0e-12){ if (verbose) {cout << '\t' << '\t' <<  '\t' << "Step size too small, psuedo-critical ray path likely." << '\n' << '\n';} break;}
        } else {
