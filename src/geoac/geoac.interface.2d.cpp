#ifndef _GEOAC_INTERFACE_2D_CPP_
#define _GEOAC_INTERFACE_2D_CPP_
...
void geoac::configure(){
    if(calc_amp){   eq_cnt = 8;}    // Use auxiliary parameters to compute amplitude
    else {          eq_cnt = 4;}    // Don't use auxiliary parameters
}
...
//---------Output The Profile---------//
//------------------------------------//
void geoac::write_prof(char* file_name, double azimuth){
	...
	if(!file_out.is_open()){
        ...
        for(int m = 0; m < alt_max * 10.0; m++){
            double z0 = topo::z(0.0) + m / 10.0;
            
            file_out << z0 << '\t';
            file_out << atmo::c(0.0, 0.0, z0) * 1000.0 << '\t';
            file_out << atmo::u(0.0, 0.0, z0) * 1000.0 << '\t';
            file_out << atmo::v(0.0, 0.0, z0) * 1000.0 << '\t';
            file_out << atmo::rho(0.0, 0.0, z0) << '\t';
            file_out << (atmo::c(0.0, 0.0, z0) + cos(azimuth) * atmo::u(0.0, 0.0, z0) + sin(azimuth) * atmo::v(0.0, 0.0, z0)) * 1000.0 << '\t';
            file_out << '\n';
        }	
        file_out.close();
    }
...
#endif /* _GEOAC_INTERFACE_2D_CPP_ */
