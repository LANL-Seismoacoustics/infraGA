...
void usage(){
    cout << '\n';
    cout << '\t' << "#########################################################" << '\n';
    cout << '\t' << "####                infraga-accel-sph                ####" << '\n';
    cout << '\t' << "####     Accelerated Ray Tracing Using Spherical     ####" << '\n';
    cout << '\t' << "####   Coordinates and a Stratified, Moving Medium   ####" << '\n';
    cout << '\t' << "#########################################################" << '\n' << '\n';
    
    cout << '\n';
    cout << "Usage: mpirun -np [number] infraga-accel-sph [option] profile.met [parameters]" << '\n';
    ...
    cout << "Examples:" << '\n';
    cout << '\t' << "mpirun -np 6 ./bin/infraga-accel-sph -prop examples/ToyAtmo.met azimuth=-90.0 write_rays=true" << '\n';
    cout << '\t' << "mpirun -np 6 ./bin/infraga-accel-sph -eig_search examples/ToyAtmo.met src_lat=30.0 src_lon=-100.0 rcvr_lat=30.25 rcvr_lon=-104.25 bnc_max=1 verbose=true" << '\n' << '\n';
}
...
void run_prop(char* inputs[], int count){
    ...
    for(double phi = phi_min;       phi <= phi_max;     phi += phi_step){
        ...
        for(double theta = max(theta_min, theta_grnd); theta <= max(theta_max, theta_grnd); theta += theta_step * world_size){
            geoac::theta = (theta + theta_step * world_rank) * (Pi / 180.0);
...
void run_eig_search(char* inputs[], int count){
    ...
    bool seq_srcs = true, write_atmo = false, custom_output_id=false, reverse_winds=false;
    ...
    if(world_rank == 0){
        cout << '\n' << "Parameter summary:" << '\n';
        cout << '\t' << "source location(s) (lat, lon, alt):" << '\n';
